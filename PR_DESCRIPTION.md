# feat: 记住每个用户的媒体播放进度

## 概述

本 PR 在保留现有播放历史功能的基础上，为本地媒体增加按用户持久化的播放进度。

用户现在可以：

- 查看电影或番剧处于未观看、观看中还是已看完状态；
- 从上次保存的位置继续播放未看完的视频；
- 在视频设置中关闭自动续播；
- 将电影、单集或整部番剧标记为看过；
- 从媒体库或单集三点菜单手动设置“正在看”“看过”或“未看”；
- 查看单集状态以及整部番剧的聚合状态；
- 退出播放器后立即看到更新后的状态，无需重新加载完整详情页。

实现中有意区分了两个概念：

- `UserHistory` 继续表示仪表盘使用的最近播放历史；
- `UserMediaProgress` 表示某个用户对某个媒体条目的持久化当前状态。

## 改动动机

改动前，视频播放器只在销毁时写入一次历史记录。这足以支持最近播放列表，但无法可靠回答以下问题：

- 应该从哪里继续播放；
- 某一集是否已经看完；
- 一部番剧中还有哪些集没有看完；
- 父级番剧是否应该视为已经看完；
- 退出播放器后如何立即更新详情页状态。

只在播放器销毁时保存也不够可靠，因为浏览器标签页、移动端 WebView 或应用进程可能在最终请求完成前被关闭。

## 用户可见行为

### 进度状态

- 尚未看完的媒体在观看进度低于 80% 时状态为 `watching`。
- 观看进度达到或超过 80% 后状态变为 `watched`。
- `watched` 是粘性状态；重播导致当前百分比回到 80% 以下时不会自动取消已看。
- 用户可以主动将媒体标记为看过。
- 手动切换未看、正在看或看过只修改状态，不会清除播放位置或改写观看百分比。

### 自动续播

- 自动续播默认开启。
- 状态为 `watching` 或 `unwatched` 且保存位置大于零的媒体会自动续播。
- 播放器会短暂显示续播提示，并提供“从头开始”操作。
- 状态为 `watched` 的媒体默认从头播放；如果保留了观看位置，会询问用户是否跳转，只有确认后才跳转。
- 该设置沿用现有的视频持久化设置；对于保存于此字段加入之前的旧设置，运行时会自动使用默认值，因此保持向后兼容。

### 番剧和单集展示

- 媒体库卡片始终显示“未看”“正在看”或“看过”之一。
- 详情页会展示父级媒体以及每个可见单集/分段的进度。
- 在播放器中选择章节时，会加载对应单集，通过统一的受控播放器流程切换视频流，并同时应用该集的保存位置和记录目标。

## 手动观看状态的详细设计

### 状态、进度与播放历史是三个不同维度

本实现没有把“是否看过”直接等同于播放百分比，也没有把手动设为“未看”等同于删除记录：

| 维度 | 保存位置 | 职责 |
| --- | --- | --- |
| 观看状态 | `UserMediaProgress.status` | 表示用户当前选择或系统判断的 `unwatched`、`watching`、`watched` |
| 播放进度 | `position`、`percentage` | 表示最后播放到多少秒、当前播放百分比，用于恢复播放 |
| 最近播放历史 | `UserHistory` | 表示用户何时播放过该媒体，用于仪表盘最近播放列表 |

三者互不替代：

- 手动切换状态不会修改 `position` 或 `percentage`；
- 手动设为未看不会删除 `UserMediaProgress`，因此以后仍能恢复原位置；
- 手动设为未看也不会删除 `UserHistory`，最近播放记录仍然保留；
- 播放器检查点会更新进度和历史，但不会把一次检查点误算为一次新的重复观看。

### 三种手动状态的写入语义

| 用户操作 | 已有进度记录 | 没有进度记录 | 是否影响单集/父级 |
| --- | --- | --- | --- |
| 标记“正在看” | 只写 `status=watching`、`manual=true`，保留位置和百分比 | 创建位置 0、百分比 0 的记录 | 只修改目标媒体 |
| 标记“看过” | 只写 `status=watched`、`manual=true`，不强制把百分比改成 100 | 创建位置 0、百分比 0 的记录 | 只修改目标媒体 |
| 标记“未看” | 只写 `status=unwatched`、`manual=true`，不删除进度 | 创建位置 0、百分比 0 的记录 | 只修改目标媒体 |

`manual` 用于区分用户明确选择与系统自动计算。用户直接设置父级番剧后，该父级状态被视为权威状态，不会因为某一集进度变化而被自动覆盖。

### 自动状态转换与“已看”粘性

播放器上报进度时会将 `manual` 恢复为 `false`，表示当前记录重新由实际播放驱动：

1. 未看或正在看的媒体开始播放后进入 `watching`；
2. 百分比第一次达到 80% 时进入 `watched`；
3. 一旦进入 `watched`，之后从头重播使百分比下降到 80% 以下，也不会自动退回 `watching`；
4. 只有用户通过手动菜单明确选择“正在看”或“未看”，才能取消粘性的已看状态；
5. 手动取消已看后，如果继续从一个已经超过 80% 的位置播放，下一次实际进度上报仍会按阈值重新判定为已看。

这样可以同时表达“这部作品曾经看完过”和“当前一次播放走到了哪里”，不会因为重播开头几秒就丢失已看事实。

### 父级番剧与单集状态相互独立

父级番剧和单集使用同一张进度表、同一套状态类型，但不是简单级联关系：

| 操作来源 | 行为 |
| --- | --- |
| 在媒体库手动设置父级番剧 | 只修改父级本身，不批量修改任何单集 |
| 在详情页手动设置某一集 | 只修改该集，然后重新计算非手动父级 |
| 播放某一集产生检查点 | 更新该集，然后重新计算非手动父级 |
| 父级已经由用户手动设置 | 保持用户选择，跳过子集自动聚合 |

自动父级聚合规则：

- 所有可见单集都有记录且都是 `watched`：父级为 `watched`，聚合百分比为 100%；
- 已有单集记录全部为 `unwatched`：父级为 `unwatched`，但仍保留按全部可见单集计算的聚合百分比；
- 其他混合情况：父级为 `watching`；
- 未产生记录的单集按 0% 参与分母，避免只看一集就让整部番剧显示过高进度；
- 没有任何单集记录时，删除自动生成的父级记录；手动父级记录不受影响。

### 不同状态的恢复播放策略

状态与进度虽然分离，但播放器会根据状态采用不同的恢复交互：

| 当前状态 | 有保存位置时的行为 |
| --- | --- |
| `watching` | 自动续播开启时直接跳转，并提供“从头开始” |
| `unwatched` | 与正在看一致：直接跳转并提供“从头开始”，因为未看状态不代表丢弃进度 |
| `watched` | 默认从头播放，只询问是否跳转到上次位置，用户确认后才跳转 |

已看跳转提示显示期间，播放器暂缓常规 `timeupdate` 检查点，避免刚从 0 秒开始播放就覆盖提示中使用的旧位置。用户确认跳转、提示超时、暂停或退出后恢复正常记录。

### 菜单入口和权限

- 媒体库卡片三点菜单向所有登录用户提供“正在看”“看过”“未看”。
- 详情页单集三点菜单向所有登录用户提供“看过”“未看”。
- 当前状态对应的菜单项会禁用，避免重复请求。
- 刮削和删除仍只对管理员显示；观看状态始终只修改当前登录用户自己的记录。
- 状态接口继续执行媒体库权限校验，普通用户不能修改未授权媒体库中的条目。

## 架构

```mermaid
sequenceDiagram
    participant Player as 视频播放器
    participant Detail as 媒体详情页
    participant API as 播放进度 API
    participant DB as SQLite

    Player->>Detail: 本地进度更新（整数秒粒度）
    Detail->>Detail: 更新内存中的当前媒体/单集
    Player->>API: 有序检查点（每 15 秒或强制事件）
    API->>DB: 更新 UserMediaProgress
    API->>DB: 在同一事务中更新 UserHistory
    API->>DB: 重新计算父级进度
    API-->>Player: 返回当前媒体进度和父级进度
    Player->>Detail: 使用服务端结果校准
```

详情页不会在每次检查点保存后重新加载完整媒体数据。本地进度负责提供即时的界面反馈，服务端响应只校准受影响的媒体条目及其父级。

## 播放生命周期

### 开始播放

1. 详情页加载媒体条目和对应进度记录。
2. 将选中的媒体和已保存进度传给 `VideoPlayer`。
3. 自动续播开启时，播放器应用保存的播放位置。
4. 第一次有效的时间更新会立即创建服务端检查点。

### 播放过程中

- 播放器根据媒体元素计算 `position` 和 `percentage`。
- 详情页最多每个整数播放秒接收一次本地更新。
- 常规服务端写入被限制为每 15 秒一次。
- 所有写入共用一条有序 Promise 队列，避免旧请求晚于新请求完成并覆盖新进度。

### 强制检查点

发生以下情况时，检查点不受 15 秒节流限制：

- 暂停播放；
- 播放结束；
- 用户切换到另一集；
- 播放器组件销毁；
- 文档进入隐藏状态；
- 页面触发 `pagehide`。

在浏览器支持时，请求会使用 `keepalive`。浏览器或 WebView 仍可能在最终请求完成前直接终止，因此周期检查点依然是必要的。

### 切换剧集

剧集切换使用统一的受控流程，不再混用插件默认切换和页面级切换：

1. 章节插件报告用户选择的章节。
2. 详情页加载目标媒体及其进度。
3. 播放器在修改活动媒体 ID 前保存上一集的最终检查点。
4. `playNext` 一次性切换视频流、标题、进度记录目标和续播位置。

对重叠的章节请求增加了顺序保护，因此较慢的旧请求不会覆盖较新的用户选择。如果服务端响应暂时落后于本地检查点，则保留更新的本地进度。

## 后端改动

### 数据库模型

新增 `user_media_progress` 表，包含以下字段：

| 字段 | 用途 |
| --- | --- |
| `user_id` | 进度记录所属用户 |
| `media_id` | 被记录的媒体条目 |
| `position` | 最后播放位置，单位为秒 |
| `percentage` | 0 到 100 的整数观看百分比 |
| `status` | `unwatched`、`watching` 或 `watched` |
| `manual` | 当前观看状态是否由用户手动设置 |
| `created_at` / `updated_at` | 标准模型时间戳 |

`(user_id, media_id)` 具有唯一约束。两个外键均使用级联删除，因此删除用户或媒体条目时也会删除对应进度。

迁移文件为 `0004_user_media_progress.py`，依赖 `0003_auto_20260531_1044`。

### API 接口

#### `POST /media/progress/list`

请求：

```json
{
  "ids": [1, 2, 3]
}
```

- 只返回当前用户自己的进度。
- 非管理员用户只能读取其获授权媒体库中的进度。
- 前端会按最多 999 个 ID 自动分批请求。

#### `POST /media/progress/record`

请求：

```json
{
  "media_id": 2,
  "position": 742,
  "percentage": 38
}
```

响应数据：

```json
{
  "progress": {},
  "parent_progress": {}
}
```

该事务会：

1. 验证媒体条目存在且属于当前用户可访问的媒体库；
2. 更新当前用户和媒体对应的进度记录；
3. 更新现有视频播放历史；
4. 在适用时重新计算父级聚合状态。

周期检查点**不会**增加 `UserHistory.repetitions`。原有历史接口继续保留其默认行为，用于真正的重复历史事件。

#### `POST /media/progress/mark`

将一个可访问的媒体条目标记为看过，并返回与记录接口相同的当前媒体/父级响应结构。该接口作为兼容入口保留，内部复用统一状态服务。

#### `POST /media/progress/status`

请求：

```json
{
  "media_id": 2,
  "status": "watching"
}
```

`status` 支持 `watching`、`watched` 和 `unwatched`：

- `watching`、`watched` 和 `unwatched` 都保留已有位置和百分比；
- 无记录时以位置和百分比 0 创建对应状态；
- `unwatched` 不删除进度记录，也不删除最近播放历史。

父级媒体的手动状态只作用于父级自身，不级联修改单集；单集状态变化会继续聚合非手动父级。

### 父级聚合

对于同一父级下的所有可见子条目：

- 如果每个子条目都有 `watched` 记录，则父级状态为 `watched`，进度为 100%；
- 如果已有子条目记录全部为 `unwatched`，则父级状态为 `unwatched`，但仍保留聚合进度；
- 其他情况父级状态为 `watching`，进度为已记录子条目百分比之和除以可见子条目总数；
- 未完成的聚合百分比最高限制为 99%；
- 手动标记的父级不会被子条目聚合覆盖，直到用户直接播放该父级并有意产生新的非手动状态。
- 如果确实不存在任何子条目进度记录，会同步删除自动生成的父级进度。

## SQLite 并发处理

容器原先使用 `--fast` 启动 Sanic，这会创建多个 worker 进程。SQLite 同一时间只允许一个写入者，并发写入进度和播放历史时会间歇性出现 `database is locked`。

本 PR 进行了两项相关调整：

1. 进度和视频播放历史通过同一个接口、同一个事务更新；
2. SQLite 容器移除 `--fast`，使用单个 Sanic worker。

权衡：单 worker 会降低 CPU 密集型请求的并行能力，并且不具备 worker 级故障接管。应用本身仍然使用异步 I/O，FFmpeg 和 aria2 也仍然运行在独立进程中。对于当前固定使用 SQLite 的配置，可靠写入比多进程请求吞吐更重要。

如果项目以后支持 PostgreSQL 或其他支持并发写入的数据库，可以只对这些后端有条件地恢复多 worker 启动。

## 兼容性

- 现有 `UserHistory` 数据不会被迁移或删除。
- 仪表盘原有历史播放功能继续工作，现在还会把媒体 ID 传给播放器。
- 旧视频设置中即使没有 `autoResume`，运行时也会默认使用 `true`。
- 网页搜索视频播放没有本地媒体 ID，因此保持原有行为。
- 进度接口要求用户登录，并遵守媒体库权限边界。
- 新表是纯增量改动，现有媒体和用户表结构不变。

## 性能考虑

- 详情页进度更新在本地完成，不会重新加载元数据、海报、演职人员或剧集列表。
- 本地界面以整数秒为粒度更新，而不是响应每一个原始 `timeupdate` 事件。
- 每个活动播放器的常规数据库写入最多每 15 秒一次。
- 检查点请求按顺序串行执行。
- 媒体列表每页使用一次批量进度请求；对于异常庞大的详情集合，会自动拆分成多个 999 ID 批次。
- 父级聚合只查询可见的同级媒体，并在进度事务内执行。

## 开发过程中已解决的审查问题

- 修复自定义章节回调阻止章节插件切换视频流的问题。
- 确保修改活动媒体 ID 前先保存上一集。
- 防止过期章节请求在较新选择完成后覆盖当前状态。
- 移除会把服务端错误伪装成“未观看”的静默进度加载失败处理。
- 为进度读取和写入增加媒体库权限过滤。
- 串行化写入，避免旧进度请求覆盖新进度。
- 防止周期检查点每 15 秒错误增加一次历史 `repetitions`。
- 播放完成或手动标记后不再重新加载整个详情页。
- 合并后端重复的 Action/Status 枚举，使模型、请求和服务共用同一个观看状态类型。
- 移除状态接口中过时的 nullable 进度分支，并把事务边界与可测试核心逻辑明确分离。
- 将播放器续播提示收敛为单一状态对象，避免提示类型、百分比和目标位置发生失配。

## 验证结果

以下检查均已成功完成：

- `npm run check`：0 个错误、0 个警告；
- `npm run build`：SvelteKit 生产构建成功；
- `ruff check app tests`：通过；
- `python -m compileall -q app tests`：通过；
- 在构建后的容器中运行后端完整测试：**282 项通过**；
- `git diff --check`：通过；
- 完整 Docker 镜像构建：通过。

前端构建仍会报告仓库原有的大体积 chunk/PWA 预缓存警告，本改动没有引入该警告。

在本地 Windows 检出中，作者原始的 `RUN chmod +x /app/entrypoint.sh` 可能受到 Docker heredoc CRLF 转换影响。仅适用于本地环境的 `sed -i 's/\r$//'` 规避方式被有意排除在本 PR 之外。最终运行时冒烟测试应在使用 LF 源文件的 Linux/CI 环境中完成。

## 人工验收清单

- [ ] 在已有工作区上应用迁移 `0004_user_media_progress`。
- [ ] 播放一个未观看的电影，退出播放器后确认详情页立即变为“正在看”。
- [ ] 再次打开该电影，确认自动从保存位置继续播放。
- [ ] 关闭自动续播，确认视频从头开始播放。
- [ ] 在续播提示中使用“从头开始”。
- [ ] 打开已看媒体，确认不会自动跳转，并可通过提示手动跳转到上次位置。
- [ ] 暂停播放并刷新页面，确认播放位置仍然保留。
- [ ] 关闭页面或将页面切到后台，确认最多只损失一个检查点周期的进度。
- [ ] 从播放器章节列表切换剧集，确认视频流、标题、记录目标和续播位置同步切换。
- [ ] 快速连续切换剧集，确认最后一次选择生效。
- [ ] 将某一集播放到 80% 以上，确认状态变为“看过”。
- [ ] 重播已经看过的媒体并回到 80% 以下，确认仍保持“看过”。
- [ ] 确认只有全部可见子条目都看完后，父级番剧才变为“看过”。
- [ ] 分别手动标记单集和父级番剧为看过。
- [ ] 在媒体库三点菜单中切换“正在看”“看过”“未看”，确认父级操作不修改单集。
- [ ] 在详情页单集三点菜单中切换“看过”和“未看”，确认父级自动聚合正确更新。
- [ ] 使用普通用户确认可见观看状态操作，但不可见刮削和删除操作。
- [ ] 在三个手动状态间切换，确认播放位置和观看百分比保持不变。
- [ ] 使用受限的非管理员用户，确认其无法读取或写入未授权媒体库的进度。
- [ ] 确认最近播放仍会出现在仪表盘中。
- [ ] 确认周期检查点不会增加历史重复次数。
- [ ] 在 Linux 上运行容器，确认 Sanic 使用单 worker 启动且不再出现 SQLite 写锁错误。

## 回滚方式

由于该功能是增量改动，应用代码可以直接回滚。如果只回滚应用而不逆向迁移，旧版本会忽略 `user_media_progress` 表。如果必须删除该表，请先备份工作区数据库，然后使用迁移框架逆向执行迁移 `0004`，或显式删除 `user_media_progress` 表。
